...
export function KeyboardHelpModal({ isOpen, onClose }: KeyboardHelpProps) {
	...
	const allShortcuts = [
		...
		{ key: 'Space', description: 'Play/pause current track', context: 'Battle' },
		...
		{ key: 'Enter', description: 'Create playlist (when ready)', context: 'Playlist' },
	...
	const formatKey = (key: string, modifier?: string) => {
		const modifierSymbol = modifier === 'ctrl' ? '⌘' : modifier === 'shift' ? '⇧' : modifier === 'alt' ? '⌥' : '';
		const keyDisplay = key === 'Space' ? 'Space' : key === 'Enter' ? '↵' : key === 'Escape' ? 'Esc' : key.toUpperCase();
					...
					>
								...
								<div key={context}>
									...
									<div className="space-y-2">
										{contextShortcuts.map((shortcut, index) => (
											<div key={`${context}-${shortcut.key}-${index}`} className="flex items-center justify-between py-2">
						...
						<div className="mt-6 pt-4 border-t border-white/20 text-center">
							<p className="text-gray-400 text-sm">
								Press <kbd className="px-2 py-1 bg-white/20 text-white rounded text-xs">/</kbd> anytime to open this help
							</p>
						</div>
					</motion.div>
				</motion.div>
